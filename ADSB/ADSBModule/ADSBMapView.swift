...
class ADSBMapView: MKMapView {
    ...
    var regionCircle: MKCircle?
    var scanRadius: CLLocationDistance? {
        didSet{
            guard scanRadius != nil else {
                return
            }
            guard scanRegionCoordinate != nil else {
                return
            }
            drawScanRegion(by: scanRadius!, at: scanRegionCoordinate!)
        }
    }
    var scanRegionCoordinate: CLLocationCoordinate2D? {
        didSet{
            guard scanRegionCoordinate != nil else {
                return
            }
...
//        altitudeStickLayer.borderColor = UIColor.blue.cgColor
        ...
        if bounds.height > 100 {
            altitudeStickLayer.frame = CGRect(x: 18, y: 40, width: 50, height: bounds.height - 80)
